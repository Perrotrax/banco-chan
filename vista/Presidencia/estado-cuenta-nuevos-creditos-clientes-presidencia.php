...
-->
    ...
    if ($Gestiones->getComprobarEstadoCuotasMensuales() == "no") {
        ...
        echo '. <strong>Por favor primero imprima este estado de cuenta. </strong>Posteriormente usted debe de generar las respectivas cuotas mensuales en nuestro sistema, este es el detalle completo que le debe ser entregado en f&iacute;sico al cliente final. Para registrar este estado de cuenta, haga clic en el bot&oacute;n <strong>[ Registrar Estado de Cuenta ]</strong>.<strong><span style="color: #8e44ad;">Tome en cuenta que esta es una acci&oacute;n que puede demorar un poco en procesarse por la cantidad de informaci&oacute;n a registrar. Por favor no realice otra acci&oacute;n sin antes mostrarse en pantalla el mensaje de confirmaci&oacute;n de nuestro sistema que el registro se ha completado con &eacute;xito.</span></strong></p>';
    } else {
        echo '<p id="aviso_empleados">Estimado(a) ';
        $Nombre = $_SESSION['nombre_usuario'];
        $PrimerNombre = explode(' ', $Nombre, 2);
    ...
    echo '
        ...
        <form ';
    if ($Gestiones->getComprobarEstadoCuotasMensuales() == "no") {
        echo 'id="ingreso-datos-cuotas-mensuales-clientes" ';
    }
    echo ' method="post">
            <input type="hidden" name="CodigoUnicoCreditoEstadoCuenta" id="CodigoUnicoCreditoEstadoCuenta" value="';
    echo $Gestiones->getIdCreditos();
    echo '">
            <input type="hidden" name="CodigoUnicoUsuarioEstadoCuenta" id="CodigoUnicoUsuarioEstadoCuenta" value="';
    echo $Gestiones->getIdUsuarios();
    echo '">
            <input type="hidden" name="CodigoUnicoProductoEstadoCuenta" id="CodigoUnicoProductoEstadoCuenta" value="';
    echo $Gestiones->getIdProductos();
    echo '">
            <input type="hidden" name="NombreProductoEstadoCuenta" id="NombreProductoEstadoCuenta" value="';
    echo $Gestiones->getNombreProductos();
    echo '">
            <input type="hidden" name="FechaPagoEstadoCuenta" id="FechaPagoEstadoCuenta" value="';
    echo $Gestiones->getFechaIngresoSolicitudCreditos();
    echo '">
            <input type="hidden" name="CuotaMensualEstadoCuenta" id="CuotaMensualEstadoCuenta" value="';
    ...
    echo '">
            <input type="hidden" name="MontoCapitalEstadoCuenta" id="MontoCapitalEstadoCuenta" value="';
    echo number_format($CalculoCuotaMensualCapital, 2);
    echo '">';
    if ($ObtenerDia >= 28 && $ObtenerDia <= 31) {
        echo '<input type="hidden" name="EvaluarDiaCalculoCuotas" value="';
        echo $ObtenerDia;
        echo '">';
    } else {
        echo '<input type="hidden" name="EvaluarDiaCalculoCuotas" value="';
    ...
    if ($Gestiones->getComprobarEstadoCuotasMensuales() == "no") {
        echo '<button id="registro_solicitud" onclick="AlertaProcesamientoDatos()" style="margin: .5rem;" type="submit" class="btn btn-rounded btn-success"><span class="btn-icon-left text-success"><i class="fa fa-upload color-success"></i></span>Registrar Estado de Cuenta</button>';
    }
    echo '
            ...
            <table style="width: 95%; margin: auto;" class="table table-striped table-responsive-sm">
                <thead class="thead-info">
                    <tr>
                        ...
                        <th>Cuota Mensual (Inter&eacute;s + Extras)</th>
                        <th>Abono Capital</th>
                        <th>Saldo Final</th>
                    </tr>
                </thead>
...
*/
    ...
    ?>
        ...
        <script src="<?php echo $UrlGlobal; ?>vista/js/alerta-registro-cuotas-mensuales-creditos-clientes.js"></script>
